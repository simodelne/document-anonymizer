import type { ReactionHandler, ReactionResult, ToolHandler } from '@simodelne/pgas-server/plugin.js';
...
export const reactionHandlers: Map<string, ReactionHandler> = new Map<string, ReactionHandler>([
  ...
  ['save_document_intake', (snapshot, trigger, mode) => {
    void trigger;
    void mode;
    return saveDocumentIntake(snapshot, true);
  }],
  ['settle_document_source', (snapshot, trigger, mode) => {
    void trigger;
    void mode;
    return settleDocumentSource(snapshot, 'inputs.source_document', 'inputs.source_document_ready', true);
  }],
]);
...
function saveDocumentIntake(snapshot: ReadonlyMap<string, unknown>, required: boolean): ReactionResult | undefined {
  if (snapshot.get('decisions.documents_received') === true) {
    return undefined;
  }
  ...
  return {
    mutations: [
      { op: 'MSet' as const, path: 'decisions.documents_received', value: true },
    ],
  };
}
...
function documentFileRefsPresent(snapshot: ReadonlyMap<string, unknown>): boolean {
  const direct = snapshot.get('inputs.document_intake.file_refs');
  if (Array.isArray(direct) && direct.length > 0) {
    return true;
  }
  const root = snapshot.get('inputs.document_intake');
  if (isDocumentRecord(root) && Array.isArray(root.file_refs) && root.file_refs.length > 0) {
    return true;
  }
  const first = snapshot.get('inputs.document_intake.file_refs.0');
  if (isDocumentRecord(first)) {
    return true;
  }
  return typeof snapshot.get('inputs.document_intake.file_refs.0.fileId') === 'string';
}

function documentSkipRequestedSnapshot(snapshot: ReadonlyMap<string, unknown>): boolean {
  const status = snapshot.get('inputs.document_intake.status');
  if (status === 'no_documents_available') {
    return true;
  }
  const root = snapshot.get('inputs.document_intake');
